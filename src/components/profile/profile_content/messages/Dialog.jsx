            ...
            <div className="messages_container">
                <div className="messages_list" ref={chatContainerRef}>
                    {messages.length > 0 ? messages.map((item, index) => (
                            <DialogItem name={item.senderId === user.items.id ? user.items.name : state?.from.name} text={item.text} date={item.createdAt}
                                        sender={item.senderId === user.items.id ? 'me' : 'client'}/>)) :
                        <p>Нет сообщений</p>}
                </div>
                <form className="dialogs-form" onSubmit={handleSubmit}>
                    ...
                    <button disabled={isLoading} type="submit"
                            className={`dialogs-sendBtn${isLoading ? ' blocked' : ''}`}>
