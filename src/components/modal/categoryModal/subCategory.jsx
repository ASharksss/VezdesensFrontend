            ...
            object.objects.map(itemObj => {
              if (count < 5) {
                ...
                return (
                  ...
                  >
										<span className='flex items-center modal_subcategory-object'
                          onClick={() => handleShowCategory(null, mainCategory)}> {itemObj.name.indexOf('/') > 1 ? itemObj.name.split('/')[0] : itemObj.name}</span>
									</span>
                );
              }
              if (count > 5) {
